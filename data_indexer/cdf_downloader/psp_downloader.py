...
class PspDownloader:
    ...
    def get_all_metadata() -> List[PspDirectoryInfo]:
        psp_filenames = [
            PspDownloader._get_metadata_for_isois(psp_isois_cda_base_url, 'ISOIS-EPIHi', 'epihi',
                                                  DefaultVariableSelector),
            PspDownloader._get_metadata_for_isois(psp_isois_cda_base_url, 'ISOIS-EPILo', 'epilo',
            ...
            PspDownloader._get_metadata_for_fields(psp_fields_cda_base_url, 'FIELDS', 'mag_rtn_4_per_cycle',
                                                   MultiDimensionVariableSelector),
            PspDownloader._get_metadata_for_fields(psp_fields_cda_base_url, 'FIELDS', 'mag_rtn_1min',
                                                   MultiDimensionVariableSelector),
        ]
    ...
    @staticmethod
    def _get_metadata_for_isois(base_url: str, instrument_human_readable: str, detector_url: str,
                                variable_selector: type[VariableSelector]) -> PspDirectoryInfo:
    ...
    @staticmethod
    def _get_metadata_for_fields(base_url: str, instrument_human_readable: str, detector_url: str,
                                 variable_selector: type[VariableSelector]) -> PspDirectoryInfo:
        url = base_url.format(detector_url)
