    ...
    def get_metadata_index():
        ...
        for psp_directory_info in psp_directory_infos:
            for category, file_infos in psp_directory_info.file_infos_by_mode.items():
                ...
                consistent_version_based_on_filenames = (
                    len(set(finfo.version for finfo in file_info_per_date.values())) == 1
                )
                for file_info in file_info_per_date.values():
                    ...
                    data_product_sources.append(
                        DataProductSource(start_time=file_start, end_time=file_end, url=file_url)
                    )
