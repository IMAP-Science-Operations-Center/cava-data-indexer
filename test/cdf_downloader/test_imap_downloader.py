...
class TestDataDownloader(TestCase):
    ...
    @patch('data_indexer.cdf_downloader.imap_downloader.ssl.create_default_context')
    @patch('data_indexer.cdf_downloader.imap_downloader.urllib')
    def test_download_metadata_from_mock_server(self, mock_urllib, mock_create_default_context):
        mock_response = Mock()
        ...
        mock_urllib.request.urlopen.assert_called_with(
            "http://3.139.73.210/dev/science-files-metadata",
    ...
    def test_download_individual_file(self, mock_urllib, mock_create_default_context):
        ...
        self.assertEqual([call("http://3.139.73.210/dev/science-files-download?file=some cdf file",
                               context=mock_create_default_context.return_value),
                          call("http://3.139.73.210/science_response",
                               context=mock_create_default_context.return_value)], mock_urllib.request.urlopen.call_args_list)
