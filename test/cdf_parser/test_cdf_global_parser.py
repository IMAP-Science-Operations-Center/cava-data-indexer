...
class TestCdfGlobalParser(unittest.TestCase):
    def test_parse_global_variables_from_isois_cdf(self):
        ...
        cases = [(date(2022, 7, 27), "test_data/test.cdf"), (None, "test_data/test_no_generation_date.cdf")]
        for generation_date, relative_cdf_path in cases:
            expected_global_info = CdfGlobalInfo(
    ...
    def test_parse_global_variables_from_fields_cdf(self):
        expected_global_info = CdfGlobalInfo(
            logical_source="psp_fld_l2_mag_RTN_4_Sa_per_Cyc",
            data_version="02",
            logical_source_description=(
                "PSP FIELDS 4 samples per cycle cadence Fluxgate Magnetometer (MAG) data in RTN coordinates"
            ),
    ...
    def test_parse_global_variables_from_omni_cdf(self):
        expected_global_info = CdfGlobalInfo(
            ...
            data_version="1",
            logical_source_description=(
                "OMNI Combined, Definitive, Hourly IMF and Plasma Data, and Energetic Proton Fluxes, "
                "Time-Shifted to the Nose of the Earth's Bow Shock, plus Solar and Magnetic Indices"
            ),
    ...
    def test_raises_value_error_if_cdf_generation_date_fails_to_be_parsed(self):
        with self.assertRaises(ValueError) as err:
            ...
            with pycdf.CDF(cdf_path) as cdf:
                CdfGlobalParser.parse_global_variables_from_cdf(cdf)

        self.assertEqual(
            "Failed to parse generation date `not a date` from CDF `unsupported_generation_date.cdf`",
            str(err.exception),
        )
