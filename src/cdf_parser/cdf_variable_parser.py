...
class CdfVariableParser:
    ...
    def _check_needed_values_are_present(variable, cdf):
        if str(cdf.attrs['Descriptor']).startswith('ISOIS-EPILO'):
            has_correct_shape = (variable.attrs['DISPLAY_TYPE'] == 'spectrogram' and len(variable.shape) <= 3) or (
                variable.attrs['DISPLAY_TYPE'] == 'time_series' and len(variable.shape) <= 2)
        else:
        ...
        zscale = variable.attrs['SCALETYP'] if 'SCALETYP' in variable.attrs else 'linear'
    ...
    def parse_info_from_cdf(cdf: pycdf.CDF) -> List[CdfVariableInfo]:
        ...
        for key, var in cdf.items():
            if var.attrs["VAR_TYPE"] == "data":
                if CdfVariableParser._check_needed_values_are_present(var, cdf):
